...
export async function deliverOneOutboxEmail(transport: SendGridTransport = sendGridTransport, now = new Date()): Promise<boolean> {
  ...
  try {
    ...
    const replyToken = createReplyToken({ conversationId: item.conversationId, inspectorId: item.inspectorId, supervisorId: item.supervisorId, expiresAt: Math.floor(now.getTime() / 1000) + 7 * 86400 }, process.env.SENDGRID_REPLY_TOKEN_SECRET!);
    ...
    await db.update(messageEmailOutboxTable).set({
      attemptCount: attempt, status: attempt >= MAX_ATTEMPTS ? "failed" : "retrying",
...
export async function drainOutbox(transport: SendGridTransport = sendGridTransport): Promise<number> {
  let sent = 0; while (await deliverOneOutboxEmail(transport)) sent++; return sent;
}